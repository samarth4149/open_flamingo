...
RUN_DB_PATH = 'expts/run_db.json'
RUN_DB_BACKUP_PATH = 'expts/run_db_backup.json'
...
def get_parser():
    ...
    parser.add_argument(
        '--local_wandb', action='store_true', default=False,
        help='Enable logging on the local wandb server'
    )
    parser.add_argument(
...
def save_expt_info(args, sha=None, outdir=None):
    ...
    with open(os.path.join(outdir, 'info.yaml'), 'w') as outfile:
